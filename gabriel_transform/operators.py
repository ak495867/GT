"""
Sublinear Operators on Gabriel Horn Representations.

Provides direct computations on compressed Gabriel Horn objects without expanding
the full n-dimensional signals:
- gabriel_point_query: O(log(1/eps)) or O(log n)
- gabriel_range_integrate: O(log n + log(1/eps))
- gabriel_inner_product: O(log(1/eps))
- gabriel_fast_convolve: Fast horn-contracted convolution
"""

from typing import Optional, Tuple
import numpy as np
from gabriel_transform.core import GabrielHornRepresentation


def _interpolate_level_at(arr: np.ndarray, norm_coord: float) -> float:
    """O(1) continuous linear interpolation of a level array at normalized coordinate [0, 1]."""
    m = len(arr)
    if m == 1:
        return float(arr[0])
    pos = norm_coord * (m - 1)
...
def gabriel_point_query(
    ...
    """
    Sublinear point query directly on GabrielHornRepresentation.

    Evaluates x[index] to accuracy epsilon by accumulating only the first
    K(eps) = O(log(1/eps)) levels.

    Returns:
        (value, levels_accessed)
    Complexity:
        O(log(1/eps)) operations.
    """
...
def gabriel_range_integrate(
    ...
    """
    Computes definite sum sum_{i=start}^{end-1} x[i] to tolerance epsilon.
    Levels beyond required depth K(eps) are omitted, giving O(log(1/eps)) level lookups.

    Returns:
        (integral_val, levels_accessed)
    """
    start = max(0, start)
...
def gabriel_inner_product(
    ...
    """
    Computes inner product <X, Y> in multiscale horn space.
    Due to geometric decay ||X_k|| <= C1 * q1^k and ||Y_k|| <= C2 * q2^k,
    the inner product terms <X_k, Y_k> decay as (q1 * q2)^k.

    Returns:
        (inner_prod, levels_computed)
    Complexity:
        O(log(1/eps)) terms.
    """
    ...
    if epsilon is not None and epsilon > 0.0:
        # Cross tail decays with product of contraction factors
...
def gabriel_fast_convolve(
    ...
    """
    Fast multiscale convolution using horn pruning.
    Interactions between fine horn tails are skipped when their cross-bound
    falls below epsilon.
    """
    k_req1 = rep1.profile.required_depth(epsilon)
    k_req2 = rep2.profile.required_depth(epsilon)

    # Reconstruct only up to required depths
